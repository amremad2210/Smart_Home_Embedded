 ...
 */
static boolean prv_memcmp(const char *buf1, const char *buf2, uint8_t length)
{
    uint8_t i;
...
{
    uint8_t result;
    char storedPassword[HAL_EEPROM_PASSWORD_MAX_LENGTH + 1U];
    ...
    /* Compare passwords */
    return prv_memcmp(password, storedPassword, length);
}
