...
void Test_Keypad_Init(void) {
    TestLogger_Assert("UT-KEYPAD-001", "Keypad driver initialization", 
                      Keypad_Init() == TRUE);
}
...
void Test_Keypad_Read(void) {
    ...
    /* Test no key pressed */
    key = Keypad_GetKey();
    TestLogger_Assert("UT-KEYPAD-002", "Keypad no key pressed", 
                      key == KEYPAD_NO_KEY || key >= '0');
}
...
void Test_Keypad_DebounceTime(void) {
    ...
    TestLogger_Assert("UT-KEYPAD-003", "Keypad debounce timing", 
                      (endTime - startTime) >= KEYPAD_DEBOUNCE_TIME);
}
